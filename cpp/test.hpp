...
struct [[nodiscard]] SomeStruct {
  public:
    ...
    int instance_var;
    inline static int static_var = 5;
...
class SomeClass : public SomeStruct {
  public:
    ...
    static int func2(const std::string& str, int* i) { return 2; };
...
void test_func() {
    ...
    int_++;
    int8_t int8 = 1;
    double double_ = 1.0 / (double)1.0f + 1.1e1 + 1e-1 + 1E+1 + 0x1p1;
    double* double_pointer = &double_;
    double deref = *double_pointer;
    double& double_ref = double_;
    char* msg = nullptr;
    const char* const msg2 = nullptr;
    msg = NULL;
    const char* msg_arr[] = {"abc", "abc\n\u0001"};
    auto strs =
        ("abc\n", L"abc\n", u8"abc\n", u"abc\n", U"abc\n", R"-(abc\n)-");
    ...
    LetterEnumClass enum2_ = LetterEnumClass::A;
    SomeUnion union_;
    union_.int_ = 5;
